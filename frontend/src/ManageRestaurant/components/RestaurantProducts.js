...
const RestaurantProducts = () => {
  ...
  const handleDelete = (id) => {
    showConfirm("Xác nhận xoá sản phẩm này?", async () => {
      try {
        const res = await fetch(`/api/restaurant/products/${id}`, {
          method: "DELETE",
          credentials: "include",
  ...
  const handleUpdateMulti = async () => {
    ...
    if (multiActionStatus === "delete-all") {
      showConfirm("Bạn có chắc chắn muốn xoá " + selectedIds.length + " sản phẩm đã chọn?", async () => {
            ...
            <div className="adm-side-body">
              <form onSubmit={handlePanelSubmit}>
                ...
                <div className="sp-form-group">
                  ...
                  {(currentProduct.img ? currentProduct.img.split(',') : ['']).map((url, i, arr) => (
                    <div key={i} style={{ display: 'flex', gap: '10px', marginBottom: '10px' }}>
                      <input type="text" className="sp-form-control" placeholder="Dán link ảnh"
                        ...
                        onChange={e => {
                          ...
                          setCurrentProduct({...currentProduct, img: newUrls.join(',')});
                        }} />
                      {arr.length > 1 && (
                        <button type="button" onClick={() => {
                          ...
                          newUrls.splice(i, 1);
                          setCurrentProduct({...currentProduct, img: newUrls.join(',')});
                        }} className="btn btn-danger" style={{ padding: '0 15px' }}><i className="bi bi-dash"></i></button>
                      )}
                    </div>
                  ))}
                  <button type="button" onClick={() => {
                    const currentUrls = currentProduct.img ? currentProduct.img.split(',') : [''];
                    currentUrls.push('');
                    setCurrentProduct({...currentProduct, img: currentUrls.join(',')});
                  }} className="adm-btn-primary" style={{ padding: '5px 10px', fontSize: '14px', background: '#34495e', marginTop: '5px' }}><i className="bi bi-plus"></i> Thêm link ảnh</button>
                </div>
