...
export default function CheckoutCartOneShop() {
    ...
    //             `/api/tables/available?restaurantId=${shopId}`
    //         );
    //         if (res.status === 401) {
    //             navigate("/user/auth/login");
    //             return;
    ...
    useEffect(() => {
        ...
        const fetchAvailableTables = async () => {
            try {
                ...
                if (formData.tableInfo.visitDate) {
                    params.append(
                        "visitDate",
                        formData.tableInfo.visitDate
                    );
                }

                if (formData.tableInfo.arrivalTime) {
                    params.append(
                        "arrivalTime",
                        formData.tableInfo.arrivalTime
                    );
                }
                ...
                setFormData((prev) => {
                    const stillExists = tables.find(
                        (table) =>
                            String(table.tableNumber) ===
                            String(prev.tableInfo.tableNumber)
                    );
    ...
    const allProducts = Array.isArray(cartItems?.products)
        ? cartItems.products
        : [];

    const products = allProducts.filter(
        (item) =>
            item.productInfo?.restaurant_id === shopId
    );
    ...
    const handleDonePay = async (event) => {
        ...
        if (
            ...
            if (!formData.tableInfo.tableNumber) {
                notifyApp(
                    "Vui lòng chọn bàn",
                    "error"
                );
        ...
        await handleLoadingCheckout(async () => {
            try {
                ...
                if (!res.ok) {
                    ...
                    if (formData.orderType === "dine_in") {
                        const tableRes = await fetch(`/api/tables/available?restaurantId=${shopId}`);
                ...
                <form className="summary-card checkout-form" onSubmit={handleDonePay}>
                    ...
                    <div className="form-grid">
                        ...
                        <label>
                            ...
                            <input
                                type="date"
                                name="visitDate"
                                value={formData.tableInfo.visitDate}
                    ...
                    <LoadingButton
                        ...
                        disabled={
                            formData.orderType === "dine_in" &&
                            (
                                !formData.tableInfo.tableNumber ||
                                !formData.tableInfo.visitDate ||
                                !formData.tableInfo.arrivalTime
                            )
                        }
